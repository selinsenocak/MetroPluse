...
/// The bottom nav's "Hesabım" tab — shows who's logged in and lets them
/// sign out back to the shared Giriş Ekranı.
class AccountScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: const MpHeader(title: 'Hesabım'),
      body: ListView(
            ...
            child: Column(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.person, color: MpColors.citizenBlue, size: 32),
                ),
                const SizedBox(height: 14),
                Text(user?.name ?? 'Misafir', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: MpColors.textDark)),
                const SizedBox(height: 4),
                if (user != null) ...[
                  Text(user.identifiers.first, style: const TextStyle(fontSize: 12.5, color: MpColors.textMuted)),
                  const SizedBox(height: 8),
                  MpBadge(text: user.roleLabel.toUpperCase(), fg: MpColors.citizenBlueDark, bg: MpColors.lightBlueBg),
                ],
              ],
            ),
          ),
          const SizedBox(height: 16),
          _menuItem(icon: Icons.description_outlined, label: 'Bildirimlerim ve Taleplerim'),
